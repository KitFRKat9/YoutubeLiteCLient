...
# ========================================
# 32bits(optimized for these systems)
# ========================================
def find_vlc():
    paths = [
        r"C:\Program Files (x86)\VideoLAN\VLC\vlc.exe",  # 32-bit VLC
        r"C:\Program Files\VideoLAN\VLC\vlc.exe"         # fallback 64-bit if it exists
    ]
    for p in paths:
        if os.path.exists(p):
            return p
...
# video search
# ========================================
def search_video(query, offset=0, limit=10):
    cmd = [
        "yt-dlp.exe",
...
def main():
    while True:
        os.system("cls")
        print("YouTubeLiteClient Lite © 2026 KitKat")
        print("Do not redistribute without credit.")
        ...
        if choice == "q":
            ...
            offset = 0
            while True:
                results = search_video(query, offset, limit=10)
                if not results:
        ...
        print("2) 480p")
        print("3) 720p (may lag on 32bits systems)")
        print("4) 1080p (may lag a lot on 32bits systems)")
